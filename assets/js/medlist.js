...
const medlistListSearchResults = document.getElementById('medlistList-searchResults');
const medlistSearchBoxClear = document.getElementById('medlistSearchBoxClear');

const loader = '<img class="mx-auto mt-5" style="width:200px; height:200px;" src="assets/images/loader.gif"></img>';
window.onload = medlistList.innerHTML = loader;

let jsonMeds = [];

async function fetchData() {  // fetch meds.json (array of objects). 

  const url = '/assets/json/meds-test.json';
...
function listMeds(data, toAppend) {   // standalone function for displaying list from 'data' js object.
  ...
  setTimeout(function generateRows(){
    ...
    for (let i = base; i < loopSize; i++) {
      ...
      text = text + `<li class="list-group-item d-flex pb-3 ps-2 ${data[i].avl ? 'list-group-item-success' : 'list-group-item-danger'}">
                      <div class="ms-2 w-100">
                        ...
                        <div class="d-flex justify-content-between text-muted pe-2">
                          ...
                          <input id="mcb" type="checkbox" data-dc="${data[i].dc}" ${data[i].avl?"checked":""}>
                        </div>
                      </div>
                    </li>`;                  
    }
...
fetchData().then(data => {
  ...
  medlistSearchBox.addEventListener('input', function (input) {
    ...
    medlistSearchBoxClear.style.display = medlistSearchBox.value.length != 0 ? 'block' : 'none';
...
btnUpdationQueue.addEventListener('click', function() {
  ...
  for (let i = 0; i < updationQueue.length; i++) {
    ...
    text = text + `<li class="list-group-item d-flex pb-3 ps-2 ${updationQueue[i].avl ? 'list-group-item-success' : 'list-group-item-danger'}">
                      <div>${i+1}. </div>
                      <div class="ms-2 w-100">
                      ...
                      <div class="d-flex justify-content-between text-muted pe-2">
                        ...
                        <input id="mcb" type="checkbox" data-dc="${updationQueue[i].dc}" ${updationQueue[i].avl?"checked":""}>
                      </div>
                    </div>
                  </li>`;  
  }
...
medlistUpdateQueueDialog.addEventListener('click', function(e) {
  ...
  if ( 'mcb' === e.target.id) {
    ...
    e.target.parentElement.parentElement.parentElement.style.transition = "opacity 0.5s ease";
